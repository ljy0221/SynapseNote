...
public interface BlockRepository extends MongoRepository<BaseBlock, String> {
    ...
    List<BaseBlock> findByNoteIdOrderByOrderAsc(UUID noteId);

    // 블록 ID로 조회
    Optional<BaseBlock> findByBlockId(UUID blockId);

    // 북마크된 블록 조회 (최신순, 페이지네이션)
    Page<BaseBlock> findByNoteIdAndBookmarkTrueOrderByUpdatedAtDesc(UUID noteId, Pageable pageable);
